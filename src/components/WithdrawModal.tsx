  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-md overflow-y-auto">
      <div className="relative w-full max-w-xl my-6 rounded-[28px] bg-[#071A0C] border border-[#7CFF00]/30 p-5 sm:p-8 shadow-2xl overflow-hidden text-white">
        ...
        <div className="absolute bottom-0 left-0 w-64 h-64 bg-[#FFB800]/10 rounded-full blur-3xl pointer-events-none" />
        ...
        {successData ? (
          ...
          <div className="mt-6 text-center py-6 space-y-4 animate-fadeIn">
            <div className="w-16 h-16 rounded-2xl bg-[#063B16] border border-[#7CFF00]/30 text-[#7CFF00] flex items-center justify-center mx-auto animate-bounce">
            ...
            <div className="p-4 rounded-2xl bg-[#040F07] border border-[#7CFF00]/15 text-xs text-[#A8B5AB] space-y-1.5 text-left max-w-sm mx-auto">
              <p>• Account Name: <strong className="text-white">{accountName}</strong></p>
              <p>• Estimated Settlement: <strong className="text-[#22C55E]">Within 5 - 15 Minutes</strong></p>
              <p>• Reference ID: <span className="font-mono text-[#7CFF00]">9JA-WD-{Date.now().toString().slice(-6)}</span></p>
            </div>
            ...
            <button
              ...
              className="w-full max-w-xs py-3.5 rounded-xl bg-gradient-to-r from-[#7CFF00] to-[#39E600] text-black font-black text-sm shadow-xl shadow-[#7CFF00]/20 transition-all cursor-pointer"
            >
              Return to Dashboard
            </button>
          </div>
          ...
          <div className="mt-5 space-y-4">
            ...
            {/* Rules Checklist Box */}
            <div className="p-4 rounded-2xl bg-[#040F07] border border-[#7CFF00]/20 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-[#A8B5AB] uppercase tracking-wider flex items-center gap-1.5">
                  <ShieldCheck className="w-4 h-4 text-[#7CFF00]" />
                  {isPremium ? 'VIP Withdrawal Rules' : 'Free Tier Withdrawal Checklist'}
                </span>
                {isWindowActive ? (
                  <span className="px-2.5 py-0.5 rounded-full bg-[#063B16] text-[#7CFF00] text-[10px] font-black border border-[#7CFF00]/30 animate-pulse">
                    ● WINDOW OPEN
                  </span>
                ) : (
              ...
              <div className="space-y-2 text-xs">
                {/* 1. Balance */}
                <div className="flex items-center justify-between p-2 rounded-xl bg-[#071A0C] border border-[#7CFF00]/10">
                  <div className="flex items-center gap-2">
                    {hasMinBalance ? (
                  ...
                  <span className={`text-[11px] font-black ${hasMinBalance ? 'text-[#22C55E]' : 'text-amber-400'}`}>
                ...
                {!isPremium && (
                  <div className="flex items-center justify-between p-2 rounded-xl bg-[#071A0C] border border-[#7CFF00]/10">
                    ...
                    <span className={`text-[11px] font-black ${hasRequiredRefs ? 'text-[#22C55E]' : 'text-amber-400'}`}>
                ...
                <div className="flex items-center justify-between p-2 rounded-xl bg-[#071A0C] border border-[#7CFF00]/10">
                  <div className="flex items-center gap-2">
                    {isWindowActive ? (
                      ...
                      <Clock className="w-4 h-4 text-[#7CFF00] flex-shrink-0" />
                    )}
                    <span className="text-gray-300">
                      {isPremium ? 'Daily Window (8:00 PM – 9:00 PM WAT):' : '29th of the Month (8:00 PM – 9:00 PM):'}
                    </span>
                  </div>
                  <span className="text-[10px] sm:text-[11px] font-mono text-[#7CFF00] font-bold">
                    {getNextWindowText()}
                  </span>
                </div>
              </div>
              ...
              {!isPremium && (
                <div className="p-3 rounded-xl bg-gradient-to-r from-amber-500/15 via-[#063B16] to-[#040F07] border border-amber-500/30 flex items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg bg-gradient-to-r from-[#FFCC33] via-[#FFB800] to-[#D99100] text-black font-black text-[11px] flex-shrink-0 shadow-md flex items-center gap-1 cursor-pointer"
                  >
            ...
            <form onSubmit={handleWithdraw} className="space-y-3.5">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="text-xs font-semibold text-[#A8B5AB]">
                    Withdrawal Amount (₦)
                  </label>
                  <button
                    type="button"
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl bg-[#040F07] border border-[#7CFF00]/15 text-white placeholder-[#526658] focus:outline-none focus:border-[#7CFF00] text-sm font-mono"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-3 rounded-xl bg-[#040F07] border border-[#7CFF00]/15 text-white focus:outline-none focus:border-[#7CFF00] text-sm cursor-pointer"
                >
                  {NIGERIAN_BANKS.map((b) => (
                    <option key={b} value={b} className="bg-[#071A0C] text-white">
                      {b}
                    </option>
                  ))}
                </select>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-[#040F07] border border-[#7CFF00]/15 text-white placeholder-[#526658] focus:outline-none focus:border-[#7CFF00] text-sm"
                  />
                </div>
              </div>
              ...
              <button
                ...
                className="w-full py-4 rounded-2xl font-black text-sm shadow-xl transition-all bg-gradient-to-r from-[#7CFF00] via-[#39E600] to-[#00B83D] hover:opacity-95 text-black shadow-[#7CFF00]/20 hover:scale-[1.01] active:scale-[0.99] cursor-pointer"
              >
                {isProcessing 
